  ...
  return (
    <Sheet onOpenChange={setOpen} open={open}>
      <SheetTrigger asChild>
        <Button className="rounded-full px-3" type="button" variant="secondary">
      ...
      <SheetPortal>
        ...
        <SheetContent className="space-y-4">
          ...
          <div className="space-y-1">
            <SheetTitle className="text-base font-semibold">Watched addresses</SheetTitle>
            <SheetDescription className="text-sm leading-6 text-slate-400">
              Switch the dashboard address saved on this device.
            </SheetDescription>
          </div>
          ...
          {accounts.length ? (
            <div className="space-y-2">
              {accounts.map(account => {
                ...
                return (
                  <div
                    className="grid grid-cols-[1fr_auto] items-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] p-2"
                    key={account.address}
                  >
                    <button
                      className="min-h-11 rounded-md px-2 text-left text-sm font-semibold text-white"
                    ...
                    <Button
                      ...
                      onClick={() => remove(account.address)}
                      size="icon"
                      type="button"
                      variant="ghost"
                    >
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button onClick={() => setIsAdding(value => !value)} type="button" variant="secondary">
              <Plus aria-hidden className="h-4 w-4" />
              Add
            </Button>
            <QrAddressScanner onAccountAdded={() => setOpen(false)} />
          </div>

          {isAdding ? <AddressEntry onAccountAdded={() => setOpen(false)} /> : null}
        </SheetContent>
      </SheetPortal>
    </Sheet>
  )
